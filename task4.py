...
# 逻辑回归模型的损失函数（对数似然损失）
def lasso_log_likelihood(X, y, weights, lambd):
    z = np.dot(X, weights)
    epsilon = 1e-10
    log_likelihood = np.sum(y * np.log(sigmoid(z) + epsilon) + (1 - y) * np.log(1 - sigmoid(z) + epsilon))
    lasso_penalty = lambd * np.sum(np.abs(weights[1:]))
    return -log_likelihood + lasso_penalty

# 梯度下降法来训练逻辑回归模型
def train(X, y, learning_rate=0.01, num_iterations=8000, lambd = 0.01):
    # 将X中的元素展平
    ...
    for i in range(num_iterations):
        ...
        lasso_gradient = lambd * np.sign(weights[1:])
        ...
        # # 每隔100次迭代输出损失值
        # if i % 100 == 0:
        #     print(f'Iteration {i}, Loss: {lasso_log_likelihood(X_bias, y, weights, lambd)}')
